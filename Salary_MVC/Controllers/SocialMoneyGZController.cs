...
{
    public class SocialMoneyGZController : ZlController
    {

        private readonly Salary_MVC.Services.SocialMoneyGZService _socialMoney = new Services.SocialMoneyGZService();
        ...
        public ActionResult GetEntity(Models.GetEntityInput parameter)
        {
            ...
            var lst = new List<DataModel.GZ_SocialMoneyDetailGZ>();
            if (master != null)
                lst = _socialMoney.GetDetailEntity(master, parameter).OrderByDescending(x => x.TotalMoney).ToList();
            master = master ?? new DataModel.GZ_SocialMoneyMasterGZ();
            var TotalCorp = lst.Sum(x => x.TotalCorp);
            var TotalPesonal = lst.Sum(x => x.TotalPesonal);
            return this.Json(new Common.OperationResult(Common.OperationResultType.Success, string.Empty, new {
                ...
                TotalCorp,
                TotalPesonal,
                TotalMoney = TotalCorp + TotalPesonal
            }));
        }
        ...
        {
            var rt = _socialMoney.ApproveByHR(parameter)?Common.OperationResultType.Success: Common.OperationResultType.Error;
        ...
        {
            var filepath = System.IO.Path.Combine(TemplateFileDirectory, "广州社保模板.xlsx");
