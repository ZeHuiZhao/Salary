...
namespace Salary_MVC.Controllers
{
    ...
    /// 深圳公积金
    /// </summary>
    public class HouseMoneyGZController : ZlController
    {
        ...
        public ActionResult GetEntity(Models.GetEntityInput parameter)
        {
            ...
            Salary_MVC.DataModel.GZ_HouseMoneyMaster master = _houseMoney.GetHouseMoneyMaster(parameter, DataModel.GZ_HouseMoneyMaster.TemplateIndex.广州);
        ...
        public ActionResult Import(Salary_MVC.Models.ImportInput parameter)
        {
            ...
            if (!DateTime.TryParse(parameter.Month, out datetime))
                throw new ArgumentException("指定的月份格式错误");
            parameter.Month = datetime.ToString("yyyy-MM");
            var rt= _houseMoney.Import(parameter, fullFilePath, Salary_MVC.DataModel.GZ_HouseMoneyMaster.TemplateIndex.广州);
        ...
        public ActionResult DeleteByMonth(string Month)
        {
            ...
            if (!DateTime.TryParse(Month, out datetime))
                throw new ArgumentException("指定的月份格式错误");
            int rt = _houseMoney.DeleteByMonth(datetime.ToString("yyyy-MM"), DataModel.GZ_HouseMoneyMaster.TemplateIndex.广州);
        ...
        {
            Salary_MVC.DataModel.GZ_HouseMoneyMaster master = _houseMoney.GetHouseMoneyMasterByFinance(parameter,DataModel.GZ_HouseMoneyMaster.TemplateIndex.广州);
            List<DataModel.GZ_HouseMoneyDetail> lst = new List<DataModel.GZ_HouseMoneyDetail>();
            if (master != null)
                lst= _houseMoney.GetHouseMoneyDetailByMasterId(master, new Models.GetEntityInput() {  Name=parameter.TabIndex== Models.TabEnum.已审核?string.Empty:parameter.Name}).OrderByDescending(x => x.Total).ToList();
            master = master ?? new DataModel.GZ_HouseMoneyMaster();
            return this.Json(new Common.OperationResult(Common.OperationResultType.Success, string.Empty, new {
                master = new { master.Id, master.Month, Status = master.Status.ToString(),StatusValue=(int)master.Status },
                data_box = lst,
        ...
        public ActionResult GetApproveLog(string Month)
        {
            ...
            if (!DateTime.TryParse(Month, out datetime))
                throw new ArgumentException("指定的月份格式错误");
            List<DataModel.GZ_ApproveLog> lst= _houseMoney.GetApproveLogByMonth(datetime.ToString("yyyy-MM"), DataModel.GZ_HouseMoneyMaster.TemplateIndex.广州);
