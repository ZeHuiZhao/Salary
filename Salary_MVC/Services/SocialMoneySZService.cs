...
{
    public class SocialMoneySZService : Service<DataModel.GZ_SocialMoneyMasterSZ>
    {
        ...
        {
            var master = this.DbContext.GZ_SocialMoneyMasterSZ.Where(x => x.Month == parameter.Month).FirstOrDefault();
            if (master != null)
                throw new ArgumentException(string.Format("已经存在{0}月的社保数据，请删除以后再上传", parameter.Month));
            var dictEmployee = this.DbContext.GZ_Employee.Select(x=> new { x.IDCard,x.Name}).ToList().Where(x => !string.IsNullOrEmpty(x.IDCard)).ToDictionary(x => x.IDCard, x => x);
            Common.AsposeSet<DataModel.GZ_SocialMoneyDetailSZ> asposeSet = new Common.AsposeSet<GZ_SocialMoneyDetailSZ>();
            asposeSet.Map((detail,cell)=>detail.Name=detail.Name);//第一列是序号，跳过
            ...
            asposeSet.Map((detail, cell) => detail.TotalMoney = decimal.Parse(cell.StringValue));
            ...
            Common.ListWrapper<DataModel.GZ_SocialMoneyDetailSZ> rtWrapper;
            ...
            master = new GZ_SocialMoneyMasterSZ() {//深圳社保主记录
                Id = Guid.NewGuid(),
            ...
            this.DbContext.GZ_SocialMoneyMasterSZ.Add(master);
            this.DbContext.GZ_SocialMoneyDetailSZ.AddRange(rtWrapper.RT);
            return this.DbContext.SaveChanges();
        }

        public bool ApproveByHR(UpdateDto parameter)
        {
            var master= this.DbContext.GZ_SocialMoneyMasterSZ.AsNoTracking().Where(x => x.Id == parameter.Id).FirstOrDefault();
            if (master == null)
            ...
            master.Status = Enum.ApproveStatus.待财务审核;
            ...
            DataModel.GZ_ApproveLog approveLog = new GZ_ApproveLog() {
                ...
                TargetTable = nameof(DataModel.GZ_SocialMoneyMasterSZ)
            };
        ...
        {
            var master= this.DbContext.GZ_SocialMoneyMasterSZ.Where(x => x.Month == Month).FirstOrDefault();
            if (master == null)
        ...
        public bool ApproveByFinance(ApproveInput parameter)
        {
            ...
            if (master.Status != Enum.ApproveStatus.待财务审核)
        ...
        /// <param name="parameter"></param>
        /// <returns></returns>
        public GZ_SocialMoneyMasterSZ GetMasterByFinance(Models.GetEntityByApprove parameter)
        {
            var query = this.DbContext.GZ_SocialMoneyMasterSZ.AsQueryable();
        ...
        /// <param name="parameter"></param>
        /// <returns></returns>
        public GZ_SocialMoneyMasterSZ GetMasterByMonth(GetEntityInput parameter)
        {
            var master = this.DbContext.GZ_SocialMoneyMasterSZ
        ...
        /// <returns></returns>
        public List<DataModel.GZ_SocialMoneyDetailSZ> GetDetailEntity(DataModel.GZ_SocialMoneyMasterSZ master, Models.GetEntityInput parameter)
        {
            var query = this.DbContext.GZ_SocialMoneyDetailSZ.Where(x => x.MasterId == master.Id);
        ...
        public int DeleteByMonth(string Month)
        {
            ...
            if(master.Status== Enum.ApproveStatus.财务同意|| master.Status== Enum.ApproveStatus.待财务审核)
            ...
            var lstDetail= this.DbContext.GZ_SocialMoneyDetailSZ.Where(x => x.MasterId == master.Id).ToList();
            this.DbContext.GZ_SocialMoneyMasterSZ.Remove(master);
            this.DbContext.GZ_SocialMoneyDetailSZ.RemoveRange(lstDetail);
            return this.DbContext.SaveChanges();
        }
    }
}